...
function StepsBoard({ stepActiveIndex, small = false }: StepsBoardProps) {
  const steps = [
    {
      step: "Step 1",
      description:
        "Choose your chef. We have verity of chefs that are expert in different cusines",
      isActive: true,
    },
    {
      step: "Step 2",
      description:
        "Customize your meal. You can book our chefs for minimum 5 hr and maximum 8hr ",
      isActive: true,
    },
    {
      step: "Step 3",
  ...
  return (
    <>
      <section
        className={`steps-board-container ${small ? "steps-board-small" : ""}`}
      >
        {steps.map((el, index) => {
          return (
            <StepsCard
